...
**Tech Stack:** Expo 54, React Native 0.81, React 19, TS 5.9, Expo Router 6, React Native Paper (MD3), Context API, React Hook Form + Zod, Axios, Clerk, i18next, AsyncStorage. Backend: FastAPI.

**Commands:**

- `npm start` (interactive dev), `npm run android`, `npm run ios`, `npm run web`, `npm run lint`
- Env vars: Copy `.env.example` to `.env` (`EXPO_PUBLIC_CLERK_PUBLISHABLE_KEY`, `EXPO_PUBLIC_API_URL`).
