...
export function mapExtraction(extraction: LeaseExtraction): MappedExtraction {
  ...
  const R = renterPrefill as Record<string, unknown>;
  for (const a of renterAssigns) {
    ...
    const source = note?.source_text ?? null;
    renterProvenance.push({ formKey: a.key, labelKey: a.i18n, prefilledValue: v, source });
    if (note) renterReview.push({ field: a.i18n, formKey: a.key, value: v, source, confidence: note.confidence });
  }
