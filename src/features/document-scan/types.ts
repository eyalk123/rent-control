 ...
 *  the RHF field name (to match the control), the value we pre-filled, the model's confidence,
 *  and the source snippet it came from. */
export interface ReviewItem {
  field: string; // i18n label key
  formKey: string; // RHF field name — matches FieldReviewProvider lookups
  value: string;
  source: string | null;
  confidence?: Confidence;
}
