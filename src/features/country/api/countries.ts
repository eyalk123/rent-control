 ...
 *
 * Mock mode answers "Israel" so the gate stays out of the way of offline UI work.
 */
export async function getMyCountry(): Promise<string | null> {
  // The dev web preview has no server. Answering "Israel" keeps the gate out of the way of
  // offline UI work, the same way the legal status does.
  if (USE_MOCK_API) return 'IL';
