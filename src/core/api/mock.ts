...
async function lockedPropertyIds(): Promise<number[]> {
  const { locked_property_ids, enforced } = await getSubscriptionMock();
...
export const mockPropertiesApi = {
  getProperties: async (): Promise<Property[]> => {
    ...
    const lockedIds = await lockedPropertyIds();
...
export const mockRentersApi = {
  getRenters: async (): Promise<Renter[]> => {
    const lockedIds = await lockedPropertyIds();
    return mockRenters.filter((r) => !r.property_id || !lockedIds.includes(r.property_id)).map((r) => {
