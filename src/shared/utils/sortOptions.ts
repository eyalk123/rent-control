 ...
 *
 * Hermes ships a partial, non-ICU `Intl` on iOS, so collator construction is guarded and
 * falls back to a plain comparison rather than throwing.
 */

export interface SortableOption {
  label: string;
  /** Sentinel rows ("All owners", "Unassigned", "Select All") stay above the sorted list. */
  pinned?: boolean;
}
...
function getCollator(lang: string): Intl.Collator | null {
  ...
  try {
    // numeric: "הרצל 2" before "הרצל 10". base: case- and niqqud-insensitive.
...
/** `sortOptions` for plain string lists (FormCreatableDropdown's owner names). */
