...
export interface Property {
   ...
   * writable. Resolved by the server per request — a fact about the account's plan today,
   * not about the property, so it is never persisted or sent back on a write.
   */
  locked?: boolean;
}
 ...
 * all rent math; these optional fields let the renter form round-trip the
 * higher-level intent (term length, renewal options, escalation rule) so an
 * edit re-opens with the same controls the user originally chose.
 */
